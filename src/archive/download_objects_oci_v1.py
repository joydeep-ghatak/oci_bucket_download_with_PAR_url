...
class ObjectDownloadPAR:
    ...
    async def async_conn_close(self):
    ...
    def __del__(self):
        ...
        try:
            if hasattr(self, "async_client") and not self.async_client.is_closed:
                ...
                asyncio.run(self.async_conn_close())
    ...
    async def _download_object(self, object):
        ...
        if os.path.isfile(local_path):
            ...
            if not os.path.isdir(local_path):
                try:
                    response = await self.async_client.get(object_url)
    ...
    async def download_objects(self,):
        self.logger.info("Starting batch download...")
        tasks = []
        async with asyncio.TaskGroup() as tg:
            for object in self.object_list:
                tasks.append(tg.create_task(self._download_object(object)))
...
def main(par_url, download_dir):
    ...
    while downloader.object_list:
        asyncio.run(downloader.download_objects())
        downloader.get_objects()
...
if __name__ == "__main__":
    ...
    # oci_par_url = "https://objectstorage.ap-mumbai-1.oraclecloud.com/p/uhYgK_VdiJjSCG8etdS2aJGfUZjnAESo9DH5peS7mZYxy-RDqnaOn-v8pWuUyn3N/n/bmehiyq3ncw4/b/model_dev_backup_bucket/o/"
